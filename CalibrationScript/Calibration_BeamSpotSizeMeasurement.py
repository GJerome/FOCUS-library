...
z_pos = np.linspace(z_axis.GetPosition(), z_axis.GetPosition() +
                    AffineTiltCorrection*PositionCube[3], Nb_pts_y)
...
for k in IteratorX:
    ...
    for j in IteratorY:
        ...
        data_Source1, t1, data_Source2, t2 = LockInDevice.AcquisitionLoop(
            Dwell_Time)
        ...
        FileControl.printProgressBar(
            IteratorY.index + 1, len(y_pos), prefix='Progress:', suffix='Complete', length=50)

    ExportData = np.array(np.transpose(
        [IntensityData, ErrorBar, IntensityData2, ErrorBar2, y_pos_real]))
    FileControl.ExportFileChopperOptimisation(
        DirectoryPath, FileNameData+'x={}'.format(str(x_axis.GetPosition())), ExportData)
    IteratorY.reset()
...
ax0.fill_between(y_pos_real, IntensityData2-ErrorBar2,
                 IntensityData2+ErrorBar2, alpha=0.5)
