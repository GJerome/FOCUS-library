...
def Find_Motor( devenum ):
    ...
    for i in range(0, 2):
        lib.get_engine_settings(axis_id_f[i], byref(eng))
        eng.MicrostepMode = MicrostepMode.MICROSTEP_MODE_FRAC_256
        lib.set_engine_settings(axis_id_f[i], byref(eng))
...
#LittleEndianStructure come from ctype library
#use to make link between dll and python
